...
private:
    ...
    void connectionHandler(std::vector<Neurons*>);
    ...
    float n_availability_decay = 0.03f; // neuron availability decay rate

    float base_amptitude = 0.8;
    float reward = 0;
    float r_r = 0.01f; //reward rate
	float r_s = 99; //reward sensitivity
    float reward_decay = 2;
    float signal_decay = 0.07; // incoming signal decay
    ...
    float weight_decay = 3650; // w decay

    float time_decay = 0.1;
    float F_T = 0.6f; //firing threshold

    float activity_record = 0;
    float a_r_d = 50; //activity record decay
    float a_r = 0.05; // activity record increment per activation

	float lowWeightTime = 0; // Time window for low weight
	float lowWeightThreshold = 50; // Threshold for low weight
    ...
    float trace_decay = 2; // Decay rate for firing trace
    ...
    float NoConnectionProbability = 0.2; // random conenction probability
